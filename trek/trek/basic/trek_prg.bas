  ...
  710 CLS : GOSUB 2400 : IF P1=>0 THEN 730 : ELSE IF ABS(P1-INT(P1+1))<0.1 THEN P1=INT(P1+1)
  720 PRINT " POWER POINTS :   "P1 : GOTO 750
  730 IF P1-INT(P1)<0.1 OR INT(P1)-P1<0.1 THEN P1=INT(P1)
 ...
 1020 IF LR=11+NP+NTT+XZ*2 THEN IF INT(P1+0.3)<>0 OR A$<>CHR$(13) THEN 830 : ELSE 1080
 1030 GOTO 830
 1040 LOCATE 1 : IF P1=>0 THEN 1060 : ELSE IF ABS(P1-INT(P1+1))<0.1 THEN P1=INT(P1+1)
 1050 PRINT " POWER POINTS :   "INT(P1*10)/10;" ";" ";"          ": GOTO 830
 1060 IF P1-INT(P1)<0.1 THEN P1=INT(P1) : ELSE IF INT(P1+1)-P1<0.1 THEN P1=INT(P1+1)
 ...
 1685 IF SD=1 THEN SOUND 2000,1 : SOUND 40,0.5 : SOUND 2000,2
 ...
 2080 IF TS=2 THEN 2090 : ELSE GOSUB 1830 : IF RND*1<0.5+PLL/4 THEN 2260
 ...
 2142 IF KB(A)>0 AND K1(A)=0 THEN K1(A)=INT(KB(A)/3+0.7)
 ...
 2400 CLO=0 : MMM=0 : MKP=KP : KT=NKT : MKP=MKP-KT*KPA : IF CLK=0 THEN 2410 : ELSE GOSUB 1830 : IF RND*1<0.7 THEN MKP=MKP-CLK : CLO=1
 ...
 2440 FOR A=1 TO B : GOSUB 1830 : B1=RND*0.6+0.7 : MZ=(MKP-NKP*MPP/3-D^0.75/KPR)*B1
 2450 GOSUB 1830 : A1=INT(RND*4+1) : IF KS(A1)>0 OR KD(A1)<0 THEN 2460 : ELSE KS(A1)=INT(MZ*KSR+0.5) : IF KS(A1)<0 THEN KS(A1)=0 : ELSE IF KS(A1)>MMP THEN KS(A1)=MMP
 2460 MKP=MKP-KS(A1)/KSR : NEXT 
 2470 MM=0 : FOR A=1 TO KBW : IF KB(A)<-9 THEN 2480 : ELSE KB(A)=0 : MM=MM+1
 2480 NEXT : FOR A=1 TO KBW : IF KB(A)<-9 OR MM<1 THEN 2490 : ELSE MN=INT(MKP/MM+0.2) : KB(A)=MN : MKP=MKP-MN : MM=MM-1
 2490 NEXT 
 ...
 3030 M1=0 : IF MP1=0 THEN 3230 : ELSE M1=1 : LOCATE 20,10 : PRINT "Movement Points : ";INT(MP1+0.8) : LOCATE 21,10 : PRINT "HELMSMAN :   Please plot our course Captain."
 ...
 3530 PRINT TAB(10);DA"unit hit to "AD$" vessel." : FOR A=1 TO 80*TR : NEXT : P=0 : SS=0 : C=100 : GOSUB 1830 : KSH=INT(RND*4+1) : KS(KSH)=KS(KSH)-DA : NTK=INT(ABS(KS(KSH))/5+0.9) : FOR A=1 TO NTK-1 : NK(A)=5 : NEXT : NK(NTK)=ABS(KS(KSH))-5*(NTK-1) : IF KS(KSH)<1 THEN 2830 : ELSE 2800
 ...
 3840 A=INT(100*(0.5+(P/P2)*100+100-((KP/KP1)*100)+(SS/SO)*100+100-(SS1/SO1)*100+C+100-C1)/6)/100 : A=A*(TK/TT)^0.5 : LOCATE 20,14 : PRINT "Your performance rating is";STR$(INT(A*100)/100);"%"
 ...
 4670 GOSUB 1830 : IF RND*1>0.5 THEN 4690
 ...
 4860 GOSUB 1830 : IF RND*1<0.5 THEN PRINT "I cannot obtain a sensors lock, Captain." : GOTO 440
 ...
 5650 IF SD=1 THEN FOR A=220 TO 400 STEP 3 : SOUND A,0.05*TR/80 : NEXT 
 ...
 5950 GOSUB 1830 : IF RND*1<0.2 THEN 5930 : ELSE IF SE1<0 OR TW<1 THEN K=L : K1=L1 : GOTO 3430
