...
25910 B=(A AND 3):KLC=0: WHILE + B>0: GOSUB 25990: IF ST(X,Y)=0 THEN KLC=KLC+1:ST(X,Y)=2+((7+ FN RAND(LEVEL))*16):B=B-1
25918 ' Generate Romulans
25920 WEND :B=(A AND 12)/4:RMC=0:RM(1,0)=10:RM(2,0)=10:RM(3,0)=10: WHILE + B>0: GOSUB 25990: IF ST(X,Y)=0 THEN RMC=RMC+1:RM(RMC,0)=X:RM(RMC,1)=Y:ST(X,Y)=1+((7+ FN RAND(LEVEL))*16):B=B-1
25938 ' Stars
25940 WEND :B= FN RAND0(LEVEL)+LEVEL: WHILE + B>0: GOSUB 25990: IF ST(X,Y)=0 THEN ST(X,Y)=5:B=B-1
25948 ' Black hole
25950 WEND :B= FN RAND0( INT (5+LEVEL)): IF B>4 THEN B=1 ELSE B=0
...
25970 WEND :B=(QT(NQTX,NQTY) AND 32):CKC=0: WHILE + B=32:CKC=1: GOSUB 25990: IF ST(X,Y)=0 THEN ST(X,Y)=6+ FN RAND(LEVEL)*16:B=0
25987 ' Planet
...
40299 '
40300 IF KLQ=1 AND NQTX<>KLX AND RSP<1000 THEN X= FN RAND(100): IF X=97 THEN OQTX=NQTX:OQTY=NQTY:NQTX=KLX:NQTY=KLY: GOSUB 20900: GOSUB 20200: PRINT SP$;"We are locked in a tractor beam";: GOSUB 23600: GOSUB 25900: GOSUB 22000
40397 '
...
43999 '
44000 IF FN RAND(1000)=997 THEN GOSUB 25990: IF X<>NQTX THEN 44010
44005 GOTO 44100
...
44199 '
44200 IF (RSP<700 AND LSA=0 AND LEVEL>4 AND FN RAND(100)>95) THEN LSA=1000:LSPD!=SDATE!+18:LSP=180: COLOR 12,4: GOSUB 20200: PRINT "                LIFE SUPPORT SYSTEM FAILING ";: COLOR 15: GOSUB 45700
44297 '
44298 ' Weaken shields
44299 '
44300 IF FN RAND(1000)=990 THEN IF SHP>90 THEN SHP=SHP-1
44500 RETURN 
44597 '
...
45499 '
45500 OUT &H3D4,4: OUT &H3D5,&H10: OUT &H3B4,4: OUT &H3B5,&H10: FOR A=1 TO 5: NEXT A: OUT &H3B4,4: OUT &H3B5,&H19: OUT &H3D4,4: OUT &H3D5,&H1F: IF SOND THEN X=1000: WHILE + X>100:A= FN RAND(100): SOUND A+X,1:X=X-X\21: SOUND 38,0: WEND : RETURN ELSE RETURN 
45597 '
...
50360 IF FN L(9)<9 THEN 50400 ELSE GOSUB 25990: IF (QT(X,Y) AND 32)=32 THEN IF KLQ=1 THEN KLQ=0:KL=KL-1
50370 KL=KL-(QT(X,Y) AND 3):RM=RM-(QT(X,Y) AND 12)/4:QT(X,Y)=64
50397 '
50398 ' Initialize game variables
50399 '
50400 SDATE!=1049.300049: TIME$ ="00:00:00":TIMES=0:TIME=0:EDATE!=SDATE!+RM*9+KL*7:CURY1=1:CURX1=1
50497 '
50498 ' Beam up STUFF
50499 '
50697 '
