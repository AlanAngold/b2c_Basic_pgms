  ...
  220 RESTORE: READ S(X,Y)
  230 NEXT Y,X
  240 FOR X=0 TO 7: FOR Y=0 TO 7: IF S(X,Y)>-1 THEN 270
  250 IF S(X,Y)=-1 FOR A=-1 TO 1 STEP 2:B=G:GOSUB 280:NEXT A
  260 IF S(X,Y)=-2 FOR A=-1 TO 1 STEP 2: FOR B=-1 TO 1 STEP 2:GOSUB280:NEXT B,A
  270 NEXT Y,X:GOTO440
  ...
  390 IF S(U+C,V+G)<0 THEN Q=Q+1:GOTO420
