...
5 PRINT "**************"
6 PRINT:PRINT
10 INPUT "HORIZONTAL LETTER LENGTH (IN INCHES)";X
20 INPUT "VERTICAL LETTER LENGTH (IN INCHES)";Y:PRINT
21 INPUT "DO YOU WANT BANNER CENTERED (ON 15 INCH PAPER)";L$:
   IF LEFT$(L$,1)="N" OR LEFT$(L$,1)="n" THEN
   PRINT "DIABLO MARGIN SET (ESCAPE 9) CAN BE USED TO SET LEFT MARGIN."
22 G1=0:IF LEFT$(L$,1)="Y" OR LEFT$(L$,1)="y" THEN G1=1
29 PRINT:PRINT "MESSAGE (USE ALL CAPITAL LETTERS)";
30 INPUT A$
31 INPUT 
   "PRINTING CHARACTER (TYPE 'ALL' IF YOU WANT CHARACTER BEING PRINTED)";M$
35 PRINT:LINE INPUT "ENTER 'RETURN' WHEN PAPER IS POSITIONED.";RR$
36 PRINT:PRINT:PRINT "---   Printing   ---":PRINT:PRINT
40 A=ASC(LEFT$(A$,1))
50 REM
60 REM
70 FOR T=1 TO LEN(A$)
...
200 RESTORE
201 X$=M$
202 IF M$="ALL" OR M$="All" OR M$="all" THEN X$=S$
205 FOR U=1 TO 7
...
280 NEXT K
445 FOR T1=1 TO X
447 LPRINT TAB((63-4.5*Y)*G1/(LEN(X$))+1);
450 FOR B=1 TO F(U)
460 IF J(B)=0 THEN 500
465 FOR I=1 TO Y: LPRINT X$;: NEXT I
470 GOTO 600
500 FOR I=1 TO Y
...
630 NEXT T1
700 NEXT U
750 FOR H=1 TO 2*X: LPRINT: NEXT H
800 NEXT T
806 LPRINT CHR$(12)
810 RUN "MENU"
812 FOR H=1 TO 7*X: LPRINT: NEXT H
813 GOTO 800
