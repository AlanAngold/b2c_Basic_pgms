0 REM Modified by Jim Woolley, FOG Disk Librarian, 8/82
1 RANDOMIZE
2 FOR A=1 TO 5:READ NA$(A):NEXT
3 DATA IBM,RCA,LBJ,ABC,CBS
5 PRINT CHR$(26):WIDTH 80
10 PRINT TAB(30);"STOCK MARKET":PRINT
...
122 PRINT "DO YOU WANT THE INSTRUCTIONS"; 
123 INPUT Z9$
124 PRINT 
125 PRINT 
...
146 PRINT "10 DAYS)":PRINT
147 PRINT "-----GOOD LUCK!-----"  :PRINT
150 LINE INPUT "HIT 'RETURN' TO START THE GAME.";XX$:PRINT:PRINT
...
339 F1$="  $$##.##":F2$="$$,###,###.##"
340 PRINT CHR$(26):PRINT "STOCK"," ","INITIALS","PRICE/SHARE"
345 PRINT "-----"," ","--------","-----------"
350 PRINT "INT. BALLISTIC MISSILES","  IBM",:PRINT USING F1$;S(1)
352 PRINT "RED CROSS OF AMERICA","  RCA",:PRINT USING F1$;S(2) 
354 PRINT "LICHTENSTEIN, BUMRAP & JOKE","  LBJ",:PRINT USING F1$;S(3)
356 PRINT "AMERICAN BANKRUPT CO.","  ABC",:PRINT USING F1$;S(4)
358 PRINT "CENSURED BOOKS STORE","  CBS",:PRINT USING F1$;S(5) 
360 PRINT 
...
397 GOTO 399 
398 PRINT "NEW YORK STOCK EXCHANGE AVERAGE: ";:PRINT USING "####.##";Z5;:
    PRINT USING "   +###.##";Z6  
399 PRINT 
400 LET T=INT(100*T+.5)/100 
401 PRINT "TOTAL STOCK ASSETS ARE   ";:PRINT USING F2$;T
403 LET C=INT(100*C+.5)/100 
405 PRINT "TOTAL CASH ASSETS ARE    ";:PRINT USING F2$;C
407 LET D=INT(100*D+.5)/100 
408 PRINT "TOTAL ASSETS ARE         ";:PRINT USING F2$;D
410 PRINT 
411 IF X9=0 THEN 416
412 LINE INPUT "DO YOU WISH TO CONTINUE?";Q9$:PRINT
413 IF LEFT$(Q9$,1)="N" THEN 998
...
658 PRINT "YOU HAVE USED ";:PRINT USING "$$##,###.##";-C5;:
    PRINT " MORE THAN YOU HAVE." 
660 GOTO 420
674 LET C=C5
...
769 PRINT      "STOCK  PRICE/SHARE  CHANGE  HOLDINGS     VALUE"
770 PRINT      "-----  -----------  ------  --------  -----------"
776 FOR XX=1 TO 5
777 PRINT USING" \ \     $$##.##    +##.##   ####   $$,###,###.##";NA$(XX),S(XX),C(XX),P(XX),S(XX)*P(XX)
778 NEXT XX
779 LET X9=1
780 PRINT 
790 PRINT 
...
995 LET A=-A
997 RETURN
998 IF D=10000 THEN PRINT "YOU BROKE EVEN." ELSE IF D>10000 THEN PRINT 
    "YOU MADE A PROFIT OF";:PRINT USING F2$;D-10000 ELSE IF D<10000 THEN PRINT 
    "YOU LOST";:PRINT USING F2$;10000-D
999 RUN "MENU"
