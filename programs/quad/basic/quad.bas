...
# gwba - GW-BASIC tokenized file (or BASICA)
  ...
  110 PRINT TAB(10);"`--------------'":GOSUB 2470:MS=RND*0.5
  ...
  350 NEXT:IF D(2)>0 AND D(2)<=1 AND SP(2)=SP1(2) THEN GOSUB 2470:SP(2)=INT(100+RND*900+0.5):PRINT E$;"I can give you";SP(2);"units of Phaser power, ";N$
  360 IF E<=20 THEN PRINT E$;"Power levels are dead, sir.":GOTO 2160
  370 IF E<150 THEN PRINT E$;"Energy Levels are Critical, ";M$;"!!!"
  380 IF Q(Q,Q1)=0 THEN G(Q,Q1)=0.1
  ...
  500 IF C=1 THEN 510 ELSE IF W1>7 AND D(0)=0 THEN MS=(INT(MS*100+0.5)/100):PRINT:PRINT E$;M$;", I think I can manage Warp";STR$(7.5+MS);".":W1=7.5+MS:SS=1:GOTO 520
  ...
  535 ED=INT(ED+0.5):PRINT:PRINT "Course plotted.  Energy drain:";STR$(ED);".":E=E-ED-EM
  ...
  770 TA=TA+0.1:E=E-X-EM:IF D(7)>0 THEN GOSUB 2470:PP=INT(20+(RND*0.6)*100):PRINT "MR. CHEKOV:  Phaser lock is malfunctioning.":PRINT TAB(14);"Phasers working at";STR$(PP);"% capacity.":X=X*(PP/100)
  780 A=A+1:GOSUB 2470:H=INT(3*((1+RND*0.2-0.4)*X/((P(A)-S)^2+(N(A)-S1)^2)^0.25+0.5)/K1):FOR B7=1 TO 3:PRINT CHR$(7);:NEXT
  ...
  820 GOSUB 2470:B7=INT(1+RND*3):GOSUB 2470:KD(A,B7)=KD(A,B7)+RND*4+0.4:PRINT "Klingon's ";D$(B7);" damaged, ";N$:KH=KH+1:GOTO 810
  ...
  970 GOSUB 2470:H=INT(250*(1+RND*0.5)+0.5):IF A$(T(A),T1(A))="*" THEN PRINT F$;"Destroying stars again, ";M$;"?":A$(T(A),T1(A))=" ":Q(Q,Q1)=Q(Q,Q1)-1:T=T-1:GOTO 1100
 ...
 1040 GOSUB 2470:B7=INT(1+RND*3):GOSUB 2470:KD(AK,B7)=KD(AK,B7)+RND*4+0.4:PRINT "Klingon's ";D$(B7);" damaged, ";N$:KH=KH+1:GOTO 1030
 ...
 1150 IF Q(A,B7)=0.1 THEN PRINT "000"; ELSE PRINT RIGHT$(STR$(Q(A,B7)+1000),3);
 ...
 1190 PRINT:PRINT O$:TA=TA+0.1:FOR C=1 TO 8:PRINTSTR$(C);:FOR D=1 TO 8
 ...
 1280 PRINT:FOR A=0 TO 10:PRINT D(A),D$(A):NEXT:TA=TA+0.1:GOTO 310
 1290 REM *** COMPUTER ***
 ...
 1530 PRINT O1$:FOR A=1 TO 8:PRINT A;:FOR B7=1 TO 8:PRINT " ";:IF G(A,B7)=0.1 OR Q(A,B7)=0.1 THEN PRINT "000";:GOTO 1550
 ...
 1600 PRINT:PRINT F$;"We have Docked.  All energy levels back to normal.":TA=TA+0.1
 ...
 1650 IF LEFT$(A$,1)="y" OR LEFT$(A$,1)="Y" THEN VA=DD*(RND*0.4+0.85):TA=TA+VA:FOR A=1 TO VA*7500:NEXT:FOR A=0 TO 10:D(A)=0:NEXT:PRINT:PRINT "Repairs completed, "N$
 ...
 1870 IF O1=0 THEN GOSUB 2470:KE(A)=INT(150+RND*351):GOSUB 2470:KT(A)=INT(RND*3+0.5):NEXT
 1880 RETURN
 ...
 1980 IF K1=0 THEN 310 ELSE B77=1:PRINT:FOR A=1 TO K1:FOR B7=1 TO 3:KD(A,B7)=KD(A,B7)-0.1:IF KD(A,B7)>0 THEN KD(A,B7)=0:NEXT:NEXT:A=1
 1990 IF KD(A,1)>0 AND KD(A,2)>0 AND KD(A,3)>0 THEN 2090
 1995 GOSUB 2470:IF RND*2<0.5 AND KD(A,1)<=0 THEN GOSUB 2530:GOTO 2080
 2000 GOSUB 2470:RR=1.25+RND*0.083333:SR=SH/RR
 2010 GOSUB 2470:IF RND*1.5<0.5 AND KT(A)>0 AND KD(A,3)<=0 THEN PRINT CHR$(7);"WARNING:  Klingon Torpedoes":KT(A)=KT(A)-1:GOSUB 2470:H=INT(200*(1+RND*0.5)+0.5):GOTO 2040
 2020 IF KD(A,2)>0 THEN 1990 ELSE GOSUB 2470:X=INT(RND*KE(A)+KE(A)/10):GOSUB 2470:H=INT(4*(1+RND*0.2-0.4)*X/((P(A)-S)^2+(N(A)-S1)^2)^0.34-X/10+0.5)
 2030 PRINT F$;"Klingons firing Phasers.":FOR B7=1 TO 3:PRINT CHR$(7);:NEXT
 2040 IF D(10)=0 THEN H=H*INT(SH/30+0.5):E=E-H:IF H<1 THEN PRINT " No damage sustained, ";N$ ELSE PRINT H;"units drained from Reserve, ";N$:X=H
 ...
 2380 IF SH=>50 AND E>2000 AND D(10)=0 THEN GOSUB 2470:SH=INT(SH*0.5):E=E-2000:A=10:DA=-2:B7=10:AK=10:GOTO 2110
 ...
 2450 IF SH=>25 AND E>1000 AND D(10)=0 THEN GOSUB 2470:SH=INT(SH*0.75):E=E-1000:A=10:DA=-1:B7=10:GOTO 2110
 ...
 2490 GOSUB 2470:MS=MS-RND*0.3:GOSUB 2470:IF RND*1.1>W3/10 THEN RETURN ELSE PRINT E$;"Engines superheating, ";M$;"!":DA=12:A=0:GOSUB 2470:A1=INT(5+RND*5):IF A1=>W3 THEN Q=Q2:Q1=Q3
 ...
 2550 IF Q(A,B7)=0 THEN G(A,B7)=0.1
 2560 RETURN
 2570 IF Q(Q,Q1)=0 THEN G(Q,Q1)=0.1:Q(Q,Q1)=0.1
