...
# gwba - GW-BASIC tokenized file (or BASICA)
  ...
  140 F=0:A#=500000.0
  150 CLS:PRINTTAB(198);"*** THE PRESIDENTIAL CAMPAIGN ***"
  ...
  450 GOSUB 1380
  460 CLS:PRINTTAB(13);"I S S U E S":PRINT
  ...
  930 ST(I,J)=ST(I,J)+INT((100-ST(I,J))*AS/1000000.0*(1/EV))
  940 IF A#<=0 THEN 980
 ...
 1280 YT=YT+EV:X=15
 1290 PRINTST$;TAB(X);EV;TAB(25);YT;TAB(31);OT
 ...
 1890 PRINT "You have been found guilty and you lose":PRINTSTR$(INT(100/(G-F)));"% of your support in each state.":GOTO 1910
 ...
 1990 FOR Y=1 TO 12
 1995 ST(2,Y)=ST(2,Y)+INT(0.07*(100-ST(2,Y)))
 1997 NEXT Y
 ...
 2400 DATA Arizona,8,6,4
 2410 DATA Arkansas,6,4,12
 2420 DATA California,52,5,6
 2430 DATA Colorado,8,6,7
 2440 DATA Conneticut,8,1,5
 2450 DATA Delaware,3,2,3
 2460 DATA D.C.,3,2,12
 2470 DATA Florida,25,4,7
 2480 DATA Georgia,13,4,8
 2490 DATA Hawaii,4,5,5
 2500 DATA Idaho,4,5,1
 2510 DATA Illinois,22,2,9
 2520 DATA Indiana,12,2,8
