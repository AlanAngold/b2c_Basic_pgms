...
# gwba - GW-BASIC tokenized file (or BASICA)
 ...
 1580 REM
 1590 C=FNX%(2)+9:IF INT(Y%/7)=Y% THEN PRINT "Seven year locusts":C=INT(C*0.65)
 1600 X1=0
 1610 FOR J1%=1 TO 5:X1=X1+U(J1%)*(1.2-0.2*J1%):NEXT J1%:IF G(8)=0 THEN C1=0:C=0:GOTO 1630
 ...
 1930 X2=INT(X2*M):X5=INT(P*X4)+7*V+13:X6=X2-4*V-INT(0.25*X5)
 1940 X2=X5-X2:L(3)=INT(0.8*X2)
 1950 IF -L(3)<=0.67*L THEN 1970
 ...
 2020 G(7)=3513:G=G+G(7):X6=-47:X4=0.55:IF K%>0 THEN 2090
 ...
 2060 PRINT "You have won the war.":X4=0.67:G(7)=INT(1.7*L(3)):G=G+G(7):GOTO 2080
 ...
 2250 P(8)=INT(P/X1):P(7)=INT(0.3-P/22):P=P+P(7)+P(8):D=D-1
 2260 G(8)=INT(C*G(8)):G=G+G(8):X1=G(8)-4000
 2270 G(9)=-G(9)*(X1<=0)+INT(0.1*X1)*(X1>0):G(9)=G(9)-120:G=G+G(9)
 ...
 2350 REM
 2360 U2=INT(U2*0.85)+U1:GOTO 720
