...
# gwba - GW-BASIC tokenized file (or BASICA)
   ...
   80 REM **** PUBLISHED IN DEC'S "101 BASIC GAMES", BY DAVID AHL.
  ...
  224 PRINT "                 ,---` '-------/ /--,"
  225 PRINT "                  `----------------'":PRINT
  226 PRINT TAB(13);"THE USS ENTERPRISE --- NCC 1701"
  260 CLEAR 600
  270 Z$="                          "
  330 DIM G(8,8),C(10,2),K(3,3),N(3),Z(8,8),D(9)
  375 GOSUB 9270:T=INT(RND*1*20+20)*100:T0=T:T9=25+INT(RND*1*10):D0=0:E=3000:E0=E
  440 P=10:P0=P:S9=200:S=0:B9=0:K9=0:X$="":X0$=" IS "
  470 DEF FND(D)=SQR((K(I,1)-S1)^2+(K(I,2)-S2)^2)
  475 GOSUB 9270:DEF FNA(R)=INT((RND*R)+1)*7.98+1.01
  480 REM ** INITIALIZE ENTERPRISE'S POSITION
  490 GOSUB 9270:Q1=INT(1+RND*7):Q2=INT(1+RND*7):GOSUB 9270:S1=INT(1+RND*7):S2=INT(1+RND*7)
  ...
  815 REM ** K3= # Klingons, B3= # Starbases, and S3= # Stars
  820 FOR I=1 TO 8:FOR J=1 TO 8:K3=0:Z(I,J)=0:GOSUB 9270:R1=RND*1
  850 IF R1>0.98 THEN K3=3:K9=K9+3:GOTO 980
  860 IF R1>0.95 THEN K3=2:K9=K9+2:GOTO 980
  870 IF R1>0.8 THEN K3=1:K9=K9+1
  980 B3=0:IF R1>0.96 THEN B3=1:B9=B9+1
 1000 GOSUB 9270:S3=INT(2+RND*7)
 1040 G(I,J)=K3*100+B3*10+S3:NEXT J:NEXT I:IF K9>T9 THEN T9=K9+1
 1100 IF B9<>0 THEN 1200
 1150 IF G(Q1,Q2)<200 THEN G(Q1,Q2)=G(Q1,Q2)+100:K9=K9+1
 1160 B9=1:G(Q1,Q2)=G(Q1,Q2)+10:GOSUB 9270:Q1=INT(1+RND*7):Q2=INT(1+RND*7)
 1200 K7=K9:IF B9<>1 THEN X$="S":X0$=" ARE "
 1230 PRINT:PRINT:PRINT:PRINT "YOUR ORDERS ARE AS FOLLOWS:"
 ...
 1280 PRINT:PRINT:PRINT:PRINT "HIT ANY KEY EXCEPT RETURN WHEN READY TO ACCEPT COMMAND"
 1300 A$=INKEY$:IF A$="" THEN 1300
 1310 REM ** HERE ANY TIME NEW QUADRANT IS ENTERED
 1320 Z4=Q1:Z5=Q2:K3=0:B3=0:S3=0:G5=0:RANDOMIZE(VAL(RIGHT$(TIME$,2))):D4=0.5*(RND*1):Z(Q1,Q2)=G(Q1,Q2)
 1390 IF Q1<1 OR Q1>8 OR Q2<1 OR Q2>8 THEN 1590
 ...
 1590 FOR I=1 TO 3:K(I,1)=0:K(I,2)=0:K(I,3)=0:NEXT I:Q$=Z$+Z$+Z$+Z$+Z$+Z$+Z$+LEFT$(Z$,17)
 1660 REM ** POSITION ENTERPRISE IN QUADRANT, THEN PLANE "K3" KLINGONS &
 ...
 1780 GOSUB 8670:K(I,1)=R1:K(I,2)=R2:GOSUB 9270:K(I,3)=S9*(0.5+(RND*1)):NEXT I
 ...
 2020 PRINT:PRINT "** FATAL ERROR **    YOU'VE JUST STRANDED YOUR SHIP IN":PRINT "SPACE"
 2030 PRINT "YOU HAVE INSUFFICIENT MANEUVERING ENERGY, AND SHIELD CONTROL"
 2040 PRINT "IS PRESENTLY INCAPABLE OF CROSS-CIRCUITING TO ENGINE ROOM!!":GOTO 6220
 ...
 2350 X$="8":IF D(1)<0 THEN X$=".2"
 2360 PRINT "WARP FACTOR (0 - "+X$+")";:INPUT W1:IF D(1)<0 AND W1>0.2 THEN 2470
 2380 IF W1>0 AND W1<=8 THEN 2490
 2390 IF W1=0 THEN 1990
 2420 PRINT "   CHIEF ENGINEER SCOTT REPORTS,  `THE ENGINES WON'T TAKE WARP";STR$(W1);"!'":GOTO 1990
 2470 PRINT "WARP ENGINES ARE DAMAGED.  MAXIMUM SPEED = WARP 0.2 .":GOTO 1990
 2490 N=INT(W1*8+0.5):IF E-N=>0 THEN 2590
 2500 PRINT "ENGINEERING REPORTS   `INSUFFICIENT ENERGY AVAILABLE":PRINT TAB(24);"FOR MANEUVERING AT WARP";STR$(W1);"!'"
 2530 IF S<N-EORD(7)<0 THEN 1990
 2550 PRINT "DEFLECTOR CONTROL ROOM ACKNOWLEDGES";S;"UNITS OF ENERGY":PRINT TAB(26);"PRESENTLY DEPLOYED TO SHIELDS."
 2570 GOTO 1990
 ...
 2770 FOR I=1 TO 8:IF D(1)=>0 THEN 2880
 2790 D(I)=D(I)+D6:IF D(I)>-0.1 AND D(I)<0 THEN D(I)=-0.1:GOTO 2880
 2800 IF D(I)<0 OR D(I)-D6=>0 THEN 2880
 ...
 2910 GOSUB 9270:R1=INT(1+RND*7):IF RND*1=>0.6 THEN 3000
 2930 D(R1)=D(R1)-((RND*1)*5+1):PRINT "DAMAGE CONTROL REPORT:  ";
 2960 GOSUB 8790:PRINT G2$;" DAMAGED":PRINT:GOTO 3070
 2970 IF D(I)=>0 OR I>8 THEN 3070
 3000 D(R1)=D(R1)+(RND*1)*3+1:IF D(I)<0 THEN PRINT "DAMAGE CONTROL REPORT:  ";
 3030 IF D(I)=>0 THEN PRINT ELSE GOSUB 8790:PRINT G2$;" STATE OF REPAIR IMPROVED":PRINT
 ...
 4270 PRINT "SCIENCE OFFICER SPOCK REPORTS  `SENSORS SHOW NO ENEMY SHIPS":PRINT TAB(33);"IN THIS QUADRANT'":GOTO 1990
 ...
 4400 IF X>E THEN 4360
 4410 E=E-X:IF D(8)<0 THEN X=X*(RND*1)
 ...
 5700 PRINT "DAMAGE CONTROL REPORT NOT AVAILABLE":IF D=0 THEN 1990
 ...
 5890 NEXT I:GOSUB 9270:T=T+D3+(0.3-RND*1)
 5910 PRINT:PRINT "DEVICE            STATE OF REPAIR":FOR R1=1 TO 8
 5920 GOSUB 8790:PRINTG2$;LEFT$(Z$,25-LEN(G2$));INT(D(R1)*100)*0.01
 ...
 6060 GOSUB 9270:H=INT((K(I,3)/FND(1))*(2+(RND*2))):S=S-H:K(I,3)=K(I,3)/(3+(RND*1))
 ...
 6120 GOSUB 9270:IF RND*2>0.6 OR H/S<=0.02 THEN 6200
 6140 GOSUB 9270:R1=INT(1+RND*7):D(INT(R1))=D(INT(R1))-H/S-0.5*(RND*2):GOSUB 8790
 6170 PRINT "DAMAGE CONTROL REPORTS `"+G2$+" DAMAGED BY THE HIT'"
 6200 NEXT I:RETURN
 6210 REM ** END OF GAME
 6220 PRINT "IT IS STARDATE";T:GOTO 6270
 6240 PRINT:PRINT "THE ENTERPRISE HAS BEEN DESTROYED.  THE FEDERATION WILL BE CONQUERED"
 ...
 6330 INPUT "LET HIM STEP FORWARD AND ENTER `AYE' ";A$:IF A$="AYE" THEN RUN
 6360 END
 6370 PRINT "CONGRATULATION, CAPTAIN!  THE LAST KLINGON BATTLE"
 6380 PRINT "CRUISER MENACING THE FEDERATION HAS BEEN DESTROYED.":PRINT
 ...
 7350 PRINT:H8=1:ON A+1 GOTO 7543,7900,8070,8500,8150,7400
 ...
 7560 O1$="     ----- ----- ----- ----- ----- ----- ----- -----"
 7570 PRINTO1$:FOR I=1 TO 8:PRINTI;:IF H8=0 THEN 7740
 7630 FOR J=1 TO 8:PRINT "   ";:IF Z(I,J)=0 THEN PRINT "***";:GOTO 7720
 7700 PRINTRIGHT$(STR$(Z(I,J)+1000),3);
 7720 NEXT J:GOTO 7850
 7740 Z4=I:Z5=1:GOSUB 9030:J0=INT(15-0.5*LEN(G2$)):PRINTTAB(J0);G2$;
 7800 Z5=5:GOSUB 9030:J0=INT(39-0.5*LEN(G2$)):PRINTTAB(J0);G2$;
 ...
 7970 IF B9<1 THEN 8010 ELSE X$="S":IF B9=1 THEN X$=""
 ...
 8060 REM ** TORPEDO, BASE NAVIGATION, D/D CALCULATOR
 ...
 8510 PRINT "MR. SPOCK REPORTS,  `SENSORS SHOW NO STARBASES IN THIS QUADRANT.'":GOTO 1990
 8580 REM ** FIND EMPTY PLACE IN QUADRANT (FOR THINGS)
 8590 GOSUB 9270:R1=INT(1+RND*7):R2=INT(1+RND*7):A$="   ":Z1=R1:Z2=R2:GOSUB 8830
 8600 RETURN
 8660 REM ** INSERT IN STRING ARRAY FOR QUADRANT
 8670 S8=INT(Z2-0.5)*3+INT(Z1-0.5)*24+1
 8673 IF A$<>"   " THEN IF MID$(Q$,S8,3)<>"   " THEN GOSUB 8590:GOTO 8670
 8675 IF LEN(A$)<>3 THEN PRINT "ERROR":STOP
 8680 IF S8=1 THEN Q$=A$+RIGHT$(Q$,189):RETURN
 8690 IF S8=190 THEN Q$=LEFT$(Q$,189)+A$:RETURN
 8700 Q$=LEFT$(Q$,ABS(S8-1))+A$+RIGHT$(Q$,ABS(190-S8)):RETURN
 ...
 8890 IF MID$(Q$,S8,3)<>A$ THEN RETURN ELSE Z3=1:RETURN
 ...
 9270 RANDOMIZE(VAL(RIGHT$(TIME$,2))):RETURN
