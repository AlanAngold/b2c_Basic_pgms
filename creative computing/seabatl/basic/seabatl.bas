...
# trs2 - TRS-80 Level II BASIC tokenized file
    5 CLS 
   10 PRINT TAB 30);"SEABATTLE"
   20 PRINT TAB 13);"CREATIVE COMPUTING  MORRISTOWN, NEW JERSEY"
   30 PRINT : PRINT : PRINT 
  270 DIM A(14,21),D(9),B(5),A$(7)
  275 RESTO : FOR RT = 1 TO 7 : READ A$(RT) : NEXT 
  280 PRINT "WHAT IS YOUR NAME FOR THE LOG, CAPTAIN";
  290 INPUT N$
  295 N$ = "CAPTAIN "+ N$
  300 PRINT 
  320 FOR I = 1 TO 14
  ...
  324 A(I,J) = 0
  326 NEXT J,I
  340 RESTO : FOR CV = 1 TO 32 : READ A$ : NEXT 
  350 FOR X = 7 TO 13
  360 FOR Y = 7 TO 12
  370 READ A(X,Y)
  380 NEXT Y
  ...
  450 RESTO : FOR CV = 1 TO 16 : READ A$ : NEXT 
  460 FOR X = 1 TO ( INT ( RND (0) * 4) + 1) * 2 - 1
  470 READ D8,D9
  480 NEXT X
  490 FOR X = 1 TO S
  ...
  730 A(X1,X2) = 6
  740 RESTO : FOR CV = 1 TO 16 : READ A$ : NEXT 
  750 FOR Y = 1 TO INT ( RND (0) * 8) + 1
  760 READ M1,M2
  770 NEXT Y
  780 NEXT X
  800 FOR I = 1 TO 9
  ...
  895 A$ = INKEY$ : IF A$ = ""THEN 895
  897 FG = 1 : GOSUB 1800 : FG = 0 : PRINT "WHAT ARE YOUR ORDERS ";N$;
  900 INPUT O
  910 ON INT (O + 1) GOTO 1050,1690,2230,2690,3260,3420,3700,3890,4410,4670
 ...
 1030 GOTO 890
 1050 X = 0 : Y = 0 : Y1 = 0 : X1 = 0 : IF D(1) > = 0 THEN 1080
 ...
 1250 IF X + X1 > 0 AND X + X1 < 15 AND Y + Y1 > 0 AND Y + Y1 < 22 THEN 1280
 ...
 1750 PRINT "OPTION #";
 1760 INPUT O
 1770 ON INT (O + 1) GOTO 1800,2020
 1780 GOTO 1750
 1800 CLS 
 1810 FOR X = 1 TO 14
 1820 FOR Y = 1 TO 21
 1830 DATA "   ",***,>V<,<R>,!H!," $ ",-#-
 1835 IF A(X,Y) < > 0 THEN 1880
 1840 IF Y < > 1 AND Y < > 21 AND X < > 1 AND X < > 14 THEN 1880
 1860 PRINT " . "; : GOTO 1950
 1880 RESTO 
 1890 ON A(X,Y) + 1 GOTO 1891,1892,1893,1894,1895,1896,1897
 1891 A$ = A$(1) : GOTO 1920
 1892 A$ = A$(2) : GOTO 1920
 1893 A$ = A$(3) : GOTO 1920
 1894 A$ = A$(4) : GOTO 1920
 1895 A$ = A$(5) : GOTO 1920
 1896 A$ = A$(6) : GOTO 1920
 1897 A$ = A$(7)
 1920 IF D < 50 AND RND (0) < .23 AND A(X,Y) < > 1 AND A(X,Y) < > 2 THEN 1860
 1930 IF RND (0) < .15 AND A(X,Y) > 2 THEN 1860
 1940 PRINT A$;
 1950 NEXT Y
 1960 PRINT 
 1970 NEXT X : IF FG = 1 THEN RETURN 
 1980 P = P - 50
 1990 IF P > 0 THEN 890
 2000 GOTO 1660
 2020 FOR I = 1 TO 5
 2022 B(I) = 0
 2024 NEXT I
 2030 PRINT "DIRECTION   # OF SHIPS    DISTANCES"
 2040 RESTO : FOR X = 1 TO 16 : READ A$ : NEXT 
 2050 FOR X = 1 TO 8
 2060 READ X1,Y1
 2070 X3 = 0
 2080 FOR X4 = 1 TO 20
 2090 IF S1 + X1 * X4 < 1 OR S1 + X1 * X4 > 14 OR S2 + Y1 * X4 < 1 OR S2 + Y1 * X4 > 21 THEN 2140
 ...
 2200 NEXT X
 2210 GOTO 1980
 2230 X = 0 : X1 = 0 : Y1 = 0 : Y = 0 : IF D(3) > = 0 THEN 2260
 ...
 2330 IF RND (0) > .5 THEN 2360
 2340 PRINT "PRESSURE IMPLODES UPON FIRING...YOU'RE CRUSHED!!"
 2350 GOTO 6190
 2360 GOSUB 6090
 2370 X = S1
 2380 Y = S2
 2390 FOR X2 = 1 TO INT (7 + 5 * ( - (D > 50)) - RND (0) * 4 + .5)
 2400 IF X + X1 > 0 AND X + X1 < 15 AND Y + Y1 > 0 AND Y + Y1 < 22 THEN 2460
 2410 PRINT "TORPEDO OUT OF SONAR RANGE...INEFFECTUAL ";N$;"."
 2420 T = T - 1
 2430 P = P - 150
 2440 IF P > 0 THEN 4700
 2450 GOTO 1660
 ...
 2730 PRINT "NOT ENOUGH CREW TO LAUNCH MISSILE ";N$;"."
 2740 GOTO 890
 ...
 2800 INPUT A$
 2810 IF LEFT$ (A$,1) = "N" THEN 890
 2820 IF RND (0) < .5 THEN 2850
 ...
 2900 GOTO 2860
 2910 F2 = INT (F1 / 75 + .5)
 2920 IF S1 + X1 * F2 > 0 AND S1 + X1 * F2 < 15 AND S2 + Y1 * F2 > 0 AND S2 + Y1 * F2 < 22 THEN 2980
 ...
 3010 IF X < 1 OR X > 14 OR Y < 1 OR Y > 21 THEN 3140
 3020 D3 = D3 - (A(X,Y) = 3)
 3030 D4 = D4 - (A(X,Y) = 6)
 3040 D5 = D5 - (A(X,Y) = 5)
 3050 D6 = D6 - (A(X,Y) = 1)
 ...
 3620 RESTO : FOR CV = 1 TO 7 : READ A$ : NEXT 
 3630 FOR X = 1 TO 9
 3640 READ A$
 3650 PRINT A$,D(X)
 3660 NEXT X
 ...
 4160 IF D3 / Q1 > 1 - RND (0) AND RND (0) + D3 / Q1 < .9 THEN 4220
 ...
 4730 IF X < 1 OR X > 14 OR Y < 1 OR Y > 21 THEN 4760
 4740 IF A(X,Y) < > 3 THEN 4760
 4750 Q = Q + ( RND (0) / SQR ((S1 - X) ^ 2 + (S2 - Y) ^ 2))
 4760 NEXT Y
 4770 NEXT X
 4780 IF Q THEN 4810
 ...
 5200 GOTO 6190
 5210 IF D(1) > = 0 OR D(3) > = 0 OR D(4) > = 0 OR D(5) > = 0 OR D(6) > = 0 OR D(7) > = 0 THEN 5270
 5220 IF D(8) > = 0 OR D(9) > = 0 THEN 5270
 5270 PRINT : PRINT : PRINT "---*** RESULT OF LAST ENEMY MANUEVER ***---"
 5280 FOR X = 1 TO 14
 5290 FOR Y = 1 TO 21
 5300 IF A(X,Y) < > 3 THEN 5700
 5320 W = D8
 5330 V = D9
 5340 IF W + X > 0 AND X + W < 15 AND Y + V > 0 AND Y + V < 22 THEN 5420
 5350 FOR X0 = 13 TO 1 STEP - 1
 5355 FOR Y0 = 20 TO 1 STEP - 1
 5360 IF A(X - W * X0,Y - V * Y0) < > 0 THEN 5400
 5370 A(X - W * X0,Y - V * Y0) = 3
 5380 A(X,Y) = 0
 5390 GOTO 6000
 5400 NEXT Y0,X0
 5410 STOP 
 5420 ON A(X + W,Y + V) + 1 GOTO 5430,5470,5530,5470,5560,5600,5650
 5430 A(X + W,Y + V) = 3
 5440 A(X,Y) = 0
 5450 GOTO 6000
 5470 RESTO : FOR CV = 1 TO 16 : READ A$ : NEXT 
 5480 FOR X0 = 1 TO INT ( RND (0) * 8) + 1
 5490 READ W,V
 5500 NEXT X0
 5510 IF X + W < 1 OR X + W > 14 OR Y + V < 1 OR Y + V > 21 THEN 5470
 5520 GOTO 5420
 5530 IF D > 50 THEN 5470
 5540 PRINT "*** YOU'VE BEEN RAMMED BY A SHIP ";N$;"!!! ***"
 5550 GOTO 6190
 5560 IF RND (0) < .15 THEN 5470
 5570 PRINT "*** YOUR HEADQUARTERS WAS RAMMED ";N$;"!!! ***"
 5580 S3 = 0 : S4 = 0 : D2 = 0 : A(X + W,Y + V) = 0
 5590 GOTO 5620
 5600 IF RND (0) < .7 THEN 5470
 5610 PRINT "*** SHIP DESTROYED BY A MINE ";N$;"!!! ***"
 5620 S = S - 1
 5630 IF S < > 0 THEN 5440
 5640 GOTO 6260
 5650 IF RND (0) < .8 THEN 5470
 5660 PRINT "*** SHIP EATEN BY A SEA MONSTER ";N$;"!! ***"
 5670 S = S - 1
 5680 GOTO 5630
 5700 IF A(X,Y) < > 6 THEN 6000
 5710 IF X + M1 < 1 OR X + M1 > 14 OR Y + M2 < 1 OR Y + M2 > 21 THEN 5770
 5720 ON A(X + M1,Y + M2) + 1 GOTO 5730,5770,5830,5850,5900,5730,5930
 5730 A(X + M1,Y + M2) = 6
 5740 A(X,Y) = 0
 5750 GOTO 6000
 5770 RESTO : FOR CV = 1 TO 16 : READ A$ : NEXT 
 5780 FOR X0 = 1 TO INT ( RND (0) * 8) + 1
 5790 READ M1,M2
 5800 NEXT X0
 5810 IF X + M1 < 1 OR X + M1 > 14 OR Y + M2 < 1 OR Y + M2 > 21 THEN 5770
 5820 GOTO 5720
 5830 PRINT "*** YOU'VE BEEN EATEN BY A SEA MONSTER ";N$;"!! ***"
 5840 GOTO 6190
 5850 IF RND (0) > .2 THEN 5770
 ...
 5910 S3 = 0 : S4 = 0 : D2 = 0
 5920 GOTO 5730
 5930 IF RND (0) < .75 THEN 5770
 5940 PRINT "*** A SEA MONSTER FIGHT ";N$;"!!! ";
 5950 IF RND (0) < .8 THEN 5980
 ...
 6130 RESTO : FOR CV = 1 TO 16 : READ A$ : NEXT 
